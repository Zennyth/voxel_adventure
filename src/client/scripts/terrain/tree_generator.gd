...
func generate() -> Structure:
	...
	# Trunk
	var trunk_len := int(random.rand_range(trunk_len_min, trunk_len_max))
	for y in trunk_len:
		voxels[Vector3(0, y, 0)] = log_type

	# Branches
	var branches_start := int(random.rand_range(trunk_len / 3, trunk_len / 2))
	for y in range(branches_start, trunk_len):
		...
		if randf() < branch_chance:
			...
			var angle := random.rand_range(-PI, PI)
