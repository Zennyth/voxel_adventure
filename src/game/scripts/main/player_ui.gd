...
# DRAG AND DROP
###
func slot_container_gui_input(event, slot_index: int):
...
func drag_stack_container(slot_index: int):
    ...
    # drop item
        ...
        if slot.get_item_name() == drag_preview.get_item_name() and slot.is_item_stackable():
            var remaining_quantity := inventory.set_stack_quantity(slot_index, drag_preview.quantity)

            if remaining_quantity == 0:
                drag_preview.set_stack(null)
            else:
                drag_preview.quantity = remaining_quantity
        
        # swap items
        else:
            drag_preview.set_stack( inventory.set_stack(slot_index, drag_preview.stack) )

func split_stack_container(slot_index: int):
    var slot: Slot = inventory.get_slot(slot_index)
...
# TOOLTIP
###
func show_item_tooltip(event, slot_index: int):
    var slot: Slot = inventory.get_slot(slot_index)
