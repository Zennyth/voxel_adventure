...
@onready var VoxelViewerPlugin := get_node("HBoxContainer/VoxelInspector/VoxelViewer")
...
func _ready():
	...
	VoxelViewerPlugin.undo_redo = undo_redo
	...
	if is_instance_valid(voxel_set):
		...
		if is_instance_valid(VoxelSetViewer):
			...
			if editing_single:
				...
				VoxelViewerPlugin.setup(voxel_set, id)
	else:
